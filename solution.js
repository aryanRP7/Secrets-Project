...
app.get("/secrets", async (req, res) => {
  ...
  if (req.isAuthenticated()) {
    try {
      ...
      if (yourSecret) {
        res.render("secrets.ejs", { secret: yourSecret });
      } else {
        res.render("secrets.ejs", { secret: "गंगाधर ही शक्तिमान है...!" });
      }
    } catch (err) {
      console.log(err);
    }
...
app.post(
  "/login",
  passport.authenticate("local", {//passport is going to trigger the strategy which we defined at end and telling we are using local strategy in order to authenticate the perticular resuest.
    successRedirect: "/secrets",
    failureRedirect: "/login",
  })
);
...
passport.use(
  "local", // here user will be verified and directed to "/secrets"
